...
export const CalendarMonth = ({consultant, newDate, month}) =>{
  let sem1 = false
  let sem2 = false
  let sem3 = false
  let sem4 = false
  ...
  const DaysOfTheWeek = () =>{
    let daysCustom = consultant.getDaysOfWeekCustom(month, newDate.year())
    return(
      <>
  ...
  const MonthsInDay = () =>{
    let mes = consultant.getAllDaysInMonth(month, newDate.year())
    let semOne = mes.slice(0,7)
    let semTwo = mes.slice(7,14)
    let semThr = mes.slice(14,21)
    let semFou = mes.slice(21,28)
    let semFive = mes.slice(28)
