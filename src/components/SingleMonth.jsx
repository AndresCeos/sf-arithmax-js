...
export const SingleMonth = ({ consultant, month, single }) => {
  ...
  const daysInMonthSingle = consultant.getAllDaysInMonth(singleMonth, newDate.year())
  const daysCustomSingle = consultant.getDaysOfWeekCustom(singleMonth, newDate.year())
  // console.log("single month => "+singleMonth);


  const currentMonthNumber = moment().month();
  const currentMonth = now.month(month - 1).format('MMMM')
  let sem1; let sem2; let sem3; let
sem4 = false
  if (newDate.date() >= 1 && newDate.date() <= 7 && newDate.year() === moment().year()) { sem1 = true }
  if (newDate.date() >= 8 && newDate.date() <= 14 && newDate.year() === moment().year()) { sem2 = true }
  if (newDate.date() >= 15 && newDate.date() <= 21 && newDate.year() === moment().year()) { sem3 = true }
  if (newDate.date() >= 22 && newDate.year() === moment().year()) { sem4 = true }
  const selectMonth = (e) => {
  ...
  return (
    <div className='p-6'>
      ...
      key={index}
      className={`
                  ${(day === newDate.date() && singleMonth === currentMonthNumber + 1 && newDate.year() === moment().year()) ? 'bg-red-80' : (newDate.year() === moment().year() && singleMonth === currentMonthNumber + 1 && (sem1 && day >= 1 && day <= 7 || sem2 && day >= 8 && day <= 14 || sem3 && day >= 15 && day <= 21 || sem4 && day >= 22)) ? 'bg-red-50' : (day % 2 == 0) ? 'bg-gray-10' : ''}
                  h-16 border border-black p-1 text-gray-500`}
                      >{day}<br /><span className='text-xl flex justify-center text-black font-bold'>{consultant.calcPersonalDay(day, singleMonth, newDate.year())}{consultant.calcPersonalDayISK(day, singleMonth, newDate.year())}/{consultant.calcUniversalDay(day, singleMonth, newDate.year())}{consultant.calcUniversalDayISK(day, singleMonth, newDate.year())}</span>
                      </div>
                    ))}
                  </div>
                </div>
