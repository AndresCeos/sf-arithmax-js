...
import {
  AnnualReturn, CircleNumber, QuaterPerMonth, QuaterPerYear,
  UnselectedConsultant
} from '../components';
import { dateSelect, useConsultant } from '../hooks';
import { sliceIntoChunks } from '../resources';
  ...
  const StageOne = () => {
    ...
    return (
      <>
        {rows.map((years, i) =>
          years.map((year, j) =>
            (<b className={`col-start-${j + 1} row-start-${i + 3} ${(year === newDate.year()) ? 'text-black' : 'text-gray-300'}`}> {year}</b>)))
        }
      </>
    )
  }
